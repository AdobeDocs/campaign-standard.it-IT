...
* Personalizza il mittente di un’e-mail. Vedi [Personalizzazione del mittente](#personalizing-the-sender).
* Personalizza l’oggetto di un’e-mail. Vedi [Personalizzazione dell’oggetto di un messaggio e-mail](../../designing/using/subject-line.md#subject-line).
...
Per definire il nome del mittente che verrà visualizzato nell’intestazione dei messaggi inviati, vai alla **[!UICONTROL Properties]** scheda della home page di E-mail Designer (accessibile tramite l’icona Home). Per ulteriori informazioni, consulta [Definizione del mittente di un’e-mail](../../designing/using/subject-line.md#email-sender).
...
1. Inserisci un URL esterno e specifica i relativi parametri. Vedi [Inserimento di un collegamento](../../designing/using/links.md#inserting-a-link).
...
* **[!UICONTROL Database URL in emails (EmailUrlBase)]**: Questo blocco di contenuto può essere utilizzato solo in un **consegna**.
* **[!UICONTROL Mirror page URL (MirrorPageUrl)]**: Questo blocco di contenuto può essere utilizzato solo in un **consegna**.
* **[!UICONTROL Link to mirror page (MirrorPage)]**: Questo blocco di contenuto può essere utilizzato solo in un **consegna**.
* **[!UICONTROL Greetings (Greetings)]**
* **[!UICONTROL Unsubscription link (UnsubscriptionLink)]**: Questo blocco di contenuto può essere utilizzato solo in un **consegna**.
* **[!UICONTROL Social network sharing links (LandingPageViralLinks)]**: Questo blocco di contenuto può essere utilizzato solo in un **pagina di destinazione**.
* **[!UICONTROL Default sender name (DefaultSenderName)]**: Questo blocco di contenuto può essere utilizzato solo in un **consegna**.
* **[!UICONTROL Name of default reply-to email address (DefaultReplyName)]**: Questo blocco di contenuto può essere utilizzato solo in un **consegna**.
* **[!UICONTROL Email address of default sender (DefaultSenderAddress)]**: Questo blocco di contenuto può essere utilizzato solo in un **consegna**.
* **[!UICONTROL Default error email address (DefaultErrorAddress)]**: Questo blocco di contenuto può essere utilizzato solo in un **consegna**.
* **[!UICONTROL Default reply-to email address (DefaultReplyAddress)]**: Questo blocco di contenuto può essere utilizzato solo in un **consegna**.
...
1. Fai clic su **[!UICONTROL Resources > Content blocks]** dal menu avanzato per accedere all’elenco dei blocchi di contenuto.
1. Fai clic sul pulsante **[!UICONTROL Create]** o duplica un blocco di contenuto preesistente.

   ![](assets/content_bloc_01.png)

1. Immetti un’etichetta.
1. Seleziona il blocco **[!UICONTROL Content type]**. Sono disponibili tre opzioni:
...
1. Puoi selezionare un **[!UICONTROL Targeting dimension]**. Per ulteriori informazioni, consulta [Informazioni sulla dimensione di targeting](#about-targeting-dimension).

   ![](assets/content_bloc_04.png)

1. È possibile selezionare la **[!UICONTROL Depends on format]** opzione per definire due blocchi diversi: uno per le e-mail di HTML e uno per le e-mail in formato testo. Verranno quindi visualizzate due schede nell’editor (HTML e testo) per definire il contenuto corrispondente.

   ![](assets/content_bloc_03.png)

1. Inserisci il contenuto dei blocchi di contenuto e fai clic sul pulsante **[!UICONTROL Create]** pulsante .
...
>[!CAUTION]
>
>Durante la modifica del contenuto di un blocco, accertati che non vi siano spazi bianchi aggiuntivi tra l’inizio e la fine del blocco *if* dichiarazioni. In HTML gli spazi bianchi vengono visualizzati sullo schermo e avranno quindi un impatto sul layout del contenuto.
...
Ad esempio, il **[!UICONTROL Unsubscription link]** la dimensione di targeting del blocco è **[!UICONTROL Profiles]** poiché contiene campi di personalizzazione specifici per **[!UICONTROL Profiles]** risorsa. Pertanto, non puoi utilizzare un **[!UICONTROL Unsubscription link]** bloccare in un [messaggio sulle transazioni degli eventi](../../channels/using/getting-started-with-transactional-msg.md#transactional-message-types), perché la dimensione di targeting di quel tipo di messaggio è **[!UICONTROL Real-time events]**. Tuttavia, puoi utilizzare la **Collegamento di annullamento dell’abbonamento** bloccare in un [messaggio sulle transazioni di profilo](../../channels/using/getting-started-with-transactional-msg.md#transactional-message-types), perché la dimensione di targeting di quel tipo di messaggio è **Profili**. Infine, la **[!UICONTROL Link to mirror page]** block non ha una dimensione di targeting, quindi puoi utilizzarla in qualsiasi messaggio.
...
1. Nella palette Proprietà immagine, seleziona la **[!UICONTROL Enable personalization]** opzione .

   ![](assets/des_personalize_images_1.png)

   La **[!UICONTROL Source]** viene visualizzato il campo e l’immagine selezionata viene visualizzata come **personalizzato** nell’editor.

1. Fai clic sulla matita accanto alla **[!UICONTROL Source]** pulsante del campo per accedere alle opzioni di personalizzazione.
...
Per aggiungere una condizione di visibilità, seleziona un blocco e immetti la condizione da rispettare nella **[!UICONTROL Visibility condition]** campo delle relative impostazioni.
...
L’editor di espressioni viene presentato nella sezione [Modifica avanzata delle espressioni](../../automating/using/editing-queries.md#about-query-editor) sezione .
...
1. Fai clic sul pulsante **[!UICONTROL Dynamic content]** dalla barra degli strumenti contestuale.

   ![](assets/dynamic_content_2.png)

   La **[!UICONTROL Dynamic content]** la sezione viene visualizzata nella palette a sinistra.
...
1. Specifica un’etichetta e seleziona i campi che desideri impostare come condizioni. Ad esempio, dal **[!UICONTROL General]** , seleziona **[!UICONTROL Age]** field
...
1. Definisci due diversi contenuti dinamici con **due condizioni diverse** ad esempio:
...
1. È quindi necessario definire la priorità per il contenuto dinamico. Una condizione con un ordine di priorità di **1** (e quindi il contenuto dinamico corrispondente) verrà inviato a un profilo anche se è presente un’altra condizione l cui ordine di priorità è **2** o **3** è soddisfatto anche da questo profilo.
...
* I contenuti dinamici vengono applicati all’immagine e questi sono configurati in base all’intervallo di età.
   ...
   L’aggiunta e la configurazione di contenuti dinamici sono descritte in [Definizione del contenuto dinamico in un messaggio e-mail](#defining-dynamic-content-in-an-email) sezione .
...
* I campi di personalizzazione e il contenuto dinamico vengono applicati al testo. A seconda dell’intervallo di età del profilo, l’e-mail inizia con il nome del profilo o con il titolo e il cognome del profilo.
   ...
   L’aggiunta e la configurazione dei campi di personalizzazione sono descritti in [Inserimento di un campo di personalizzazione](#inserting-a-personalization-field) sezione .
...
1. Seleziona il contenuto dinamico nel **[!UICONTROL Properties]** e fai clic su **[!UICONTROL Edit]** pulsante .

   ![](assets/delivery_content_48.png)

1. Modifica l’etichetta e seleziona la **[!UICONTROL Age]** dal campo **[!UICONTROL Profile]** nodo.

   ![](assets/delivery_content_49.png)

1. Seleziona la **Maggiore o uguale a** operatore quindi immettere **18** per creare **più di 18 anni** espressione.

   ![](assets/delivery_content_50.png)

1. Aggiungi un nuovo **[!UICONTROL Age]** condizione.

   Seleziona la **Minore o uguale a** nel campo value , seguito da 27 per creare il **meno di 27 anni** espressione.
...
1. Modifica l’etichetta e seleziona la **[!UICONTROL Age]** dal campo **[!UICONTROL Profile]** nodo.
1. Aggiungi il **Maggiore di** nel campo value , seguito da 27 per creare il **più di 27 anni** espressione.
...
1. Nell’elenco visualizzato, seleziona la **[!UICONTROL First name]** e conferma.
...
1. Procedi in modo simile per aggiungere il **[!UICONTROL Last name]** campo .
